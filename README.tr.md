...
- `cutloc stop` ve `cutloc restart`, ortak sunucunun yaşam döngüsünü açıkça yönetir; canlı komutlar uyumsuz kalan yönetilen sunucuyu otomatik yeniler. Stop/restart etkin medya veya önizleme işlerini kesmez; etkin editör oturumlarını kapatmak için açıkça `--force` gerekir. Açık tarayıcı ilerleme bağlantıları kapanış sırasında temizce sonlandırılır.
