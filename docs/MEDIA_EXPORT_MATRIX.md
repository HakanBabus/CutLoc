...
- Uzantı, bildirilen MIME ve ffprobe container/format sonucu uyuşmak zorundadır; örneğin JPEG byte'ları `.png` adıyla kabul edilmez.
...
- Relink gerçek uzantıyı, MIME ve tüm medya metadata alanlarını atomik günceller; daha kısa replacement mevcut source range'leri karşılamıyorsa eski medya korunarak reddedilir.
- Library'den kaldırılan asset, referans veren kliplerle birlikte project JSON'dan ve source/proxy/thumbnail/waveform dosyalarıyla birlikte diskten kaldırılır.
...
| Clip / speed curve hızı | `0.25x`–`4x` |

Geçersiz veya eksik In/Out range full timeline'a sessizce düşmez. Muted track sesleri audio export'a katılmaz; track volume, clip volume ve volume keyframe değerleri birlikte uygulanır. Aynı isimle eşzamanlı export istekleri ayrı output path'leri rezerve eder.
