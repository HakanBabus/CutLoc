...
- Aynı proje iki sekmede açıksa bağımsız alan değişiklikleri kayıt sırasında uzlaştırılır; aynı alan için çakışan editler sessiz `last writer wins` ile üzerine yazılmaz ve recovery mesajı gösterilir.
- Local CLI aynı proje için özel bir erişim lease'i aldığında web editörü salt okunur duruma geçer ve lease sahibini gösterir. Lease heartbeat kesilirse süre sonunda otomatik kalkar.
- CLI proje editleri doğrudan disk yazımıyla değil, web ile aynı local API, revision ve `ProjectSchema` doğrulama sınırı üzerinden yapılır.
...
- Proje doğrulaması unique kimlikleri, asset referanslarını, source range sınırlarını, keyframe/speed-point zamanlarını ve timeline duration invariant'ını da kontrol eder.
