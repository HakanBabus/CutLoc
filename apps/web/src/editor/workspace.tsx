...
export function Editor({ onBack }: { onBack: () => void }) {
  ...
  const persistWorkspaceLayout = (next: WorkspaceLayout) => {
    ...
    void api<Settings>('/api/settings', {
      method: 'PUT',
      body: JSON.stringify({ workspaceLayout: next }),
    }).catch(() => setEditorNotice(t('editor.layoutSaveFailed')));
  };
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (!event.ctrlKey && !event.metaKey && !event.altKey && event.key.toLocaleLowerCase() === 'm') {
        event.preventDefault();
      ...
      if (matchesShortcut(event, shortcutValue(settings, 'undo'))) {
        event.preventDefault();
        undo();
      }
      if (matchesShortcut(event, shortcutValue(settings, 'redo')) || ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'y')) {
        event.preventDefault();
        redo();
      }
      if (matchesShortcut(event, shortcutValue(settings, 'selectAll'))) {
        event.preventDefault();
        useEditor.getState().setSelectedMany(
          project.tracks
      ...
      if (matchesShortcut(event, shortcutValue(settings, 'togglePlayback'))) {
        event.preventDefault();
        useEditor.getState().setPlaying(!useEditor.getState().playing);
      }
      if (matchesShortcut(event, shortcutValue(settings, 'deleteClip')) && selectedClipId) {
        event.preventDefault();
        mutateProject((draft) => {
          for (const track of draft.tracks) track.clips = track.clips.filter((clip) => !selectedClipIds.includes(clip.id));
          draft.duration = projectDuration(draft);
        });
      ...
      if (matchesShortcut(event, shortcutValue(settings, 'split')) && selectedClipId) {
        event.preventDefault();
        mutateProject((draft) => {
          splitClipAt(draft, selectedClipId, useEditor.getState().currentTime);
        });
      }
      ...
      if (matchesShortcut(event, shortcutValue(settings, 'setIn'))) {
        event.preventDefault();
        setRangeStart(useEditor.getState().currentTime);
      }
      if (matchesShortcut(event, shortcutValue(settings, 'setOut'))) {
        event.preventDefault();
        setRangeEnd(useEditor.getState().currentTime);
      }
      if (matchesShortcut(event, shortcutValue(settings, 'clearRange'))) {
        event.preventDefault();
        clearRange();
      }
    };
  ...
  const importMedia = async (file: File) => {
    ...
    try {
      ...
      setExportMessage(error instanceof Error ? error.message : t('editor.mediaImportFailed'));
    }
  };
  ...
  const watchExportJob = (jobId: string) =>
          ...
          .catch(() => {
            if (!settled && !source)
              setExportStatus((current) => ({
                ...
                jobId,
                status: 'reconnecting',
                message: t('export.serverReconnecting'),
              }));
          })
          .finally(() => {
      ...
      const connect = () => {
        ...
        source.onerror = () => {
          ...
          setExportStatus((current) => ({
            ...
            jobId,
            status: 'reconnecting',
            message: t('export.connectionReconnecting'),
          }));
  ...
  const startExportResilient = async (options: ExportOptions): Promise<ExportPreflight> => {
    ...
    try {
      setExportStatus({
        progress: 0,
        status: 'saving',
        message: t('export.verifyingProjectSave'),
      });
      const confirmedProject = await ensureProjectSaved();
      const requestBody = {
        ...options,
        projectRevision: confirmedProject.revision,
      };
      setExportStatus({
        progress: 0,
        status: 'preflight',
        message: t('export.preflightRunning'),
      });
      ...
      setExportStatus({
        progress: 0,
        status: 'failed',
        error: error instanceof Error ? error.message : t('export.failedToStart'),
      });
      throw error;
    }
  };
  ...
  const commandActions: CommandAction[] = [
    {
      id: 'media',
      label: t('command.media'),
      icon: '▧',
      shortcut: 'M',
      run: () => useEditor.getState().setPanel('media'),
    },
    {
      id: 'text',
      label: t('command.text'),
      icon: 'T',
      run: () => useEditor.getState().setPanel('text'),
    },
    {
      id: 'animation',
      label: t('command.animation'),
      icon: '✧',
      run: () => useEditor.getState().setInspectorTab('motion'),
    },
    {
      id: 'project',
      label: t('command.project'),
      icon: '◉',
      run: () => useEditor.getState().setPanel('project'),
    },
    {
      id: 'playback',
      label: t('command.playback'),
      icon: '▶',
      shortcut: shortcutValue(settings, 'togglePlayback'),
      run: () => useEditor.getState().setPlaying(!useEditor.getState().playing),
    },
    {
      id: 'export',
  ...
  return (
    <div className="editor-shell">
      ...
      <div
        className="editor-body workspace-layout"
        style={
          {
            '--workspace-rail-width': `${workspaceLayout.railWidth}px`,
            '--workspace-library-width': `${workspaceLayout.libraryWidth}px`,
            '--workspace-inspector-width': `${workspaceLayout.inspectorWidth}px`,
            '--workspace-timeline-height': `${workspaceLayout.timelineHeight}px`,
          } as React.CSSProperties
        }
      >
        <ToolRail onOpenSettings={() => setShowSettings(true)} />
        <AssetPanelPro onImport={importMedia} onOpenSettings={() => setShowSettings(true)} />
        <PreviewArea project={project} settings={settings} />
        <Inspector project={project} />
        <TimelinePro project={project} />
        <WorkspaceResizers layout={workspaceLayout} onPreview={setWorkspaceLayout} onCommit={persistWorkspaceLayout} />
      </div>
      {exportMessage && (
        <div className={`export-toast ${exporting ? 'active' : ''}`}>
          <span className="export-pulse" />
          {exportMessage}
          {!exporting && <button onClick={() => setExportMessage('')}>×</button>}
        </div>
      )}
      {editorNotice && (
        <div className="export-toast editor-notice" role="status" aria-live="polite">
          <span className="editor-notice-icon" aria-hidden="true">
            i
          </span>
          <span className="editor-notice-copy">{editorNotice}</span>
          <button onClick={() => setEditorNotice('')} aria-label={t('common.close')}>
            ×
          </button>
        </div>
      )}
      <div className="editor-statusbar">
        <span>
          <i className="status-dot" /> {t('editor.status.ready')}
        </span>
      </div>
      {projectAccess && (
        <div className="cli-access-lock" role="alert" aria-live="assertive">
          <section>
            <span className="cli-access-badge">CLI</span>
            <h2>{t('editor.access.cliTitle')}</h2>
            <p>{t('editor.access.cliCopy', { owner: projectAccess.ownerLabel })}</p>
            <small>{t('editor.access.cliHint')}</small>
          </section>
        </div>
      )}
...
function ExportModal({ project, settings, rangeStart, rangeEnd, exporting, status, onStart, onCancel, onAddFirstAsset, onClose }: { project: Project; settings: Settings | null; rangeStart: number | null; rangeEnd: number | null; exporting: boolean; status: ExportStatus; onStart: (options: ExportOptions) => Promise<ExportPreflight>; onCancel: () => Promise<void>; onAddFirstAsset: () => boolean; onClose: () => void }) {
  ...
  const outputSize = exportDimensions(aspect, resolution, {
  ...
  const submit = async () => {
    ...
    try {
      setPreflight(await onStart(options()));
    } catch (submitError) {
      setError(submitError instanceof Error ? submitError.message : t('export.failedToStart'));
    }
  };
  return (
    <div
      ...
      <section ref={dialogRef} className={`export-modal ${showSuccess ? 'export-modal-complete' : ''}`} role="dialog" aria-modal="true" aria-labelledby="export-title" tabIndex={-1} onKeyDown={handleDialogKeyDown}>
        <div className="modal-head">
          <div>
            ...
            <small>
              {project.name} ·{' '}
              {t('export.timelineDuration', {
                duration: formatTime(project.duration, true, project.canvas.fps),
              })}
            </small>
          </div>
          <button onClick={onClose} disabled={exporting} aria-label={t('common.close')}>
            ×
          </button>
        </div>
        {showSuccess && (
          ...
          <div className="export-form">
            ...
            <div className="export-section">
              ...
              <div className="export-canvas-readonly export-canvas-profile">
                <span>{t('export.canvasLocked')}</span>
                <strong>
                  {aspect} · {t('export.canvas')} {canvasHint}
                </strong>
                <small>
                  {isVideo
                    ? t('export.output', {
            ...
            <div className="export-grid-row">
              <label>
                ...
                <select value={format} onChange={(event) => setFormat(event.target.value as ExportOptions['format'])} disabled={exporting}>
              ...
              {isVideo ? (
                ...
                <div className="export-format-note">
                  <span>{t('export.outputType')}</span>
                  <strong>{format === 'wav' ? t('export.losslessAudio') : t('export.compressedAudio')}</strong>
                </div>
              )}
            </div>
            <div className="export-grid-row">
              ...
              {usesCompressedAudio && (
                <label>
                  <span>{t('export.audioBitrate')}</span>
                  <select value={audioBitrateKbps} onChange={(event) => setAudioBitrateKbps(Number(event.target.value) as 128 | 192 | 256)} disabled={exporting}>
              ...
              {!isVideo && !usesCompressedAudio && (
                <div className="export-format-note export-format-note-wide">
                  <span>{t('export.audioBitrate')}</span>
                  <strong>{t('export.pcmAudio')}</strong>
                </div>
              )}
            </div>
            {isVideo && (
              <div className="export-section">
                <span className="export-label">{t('export.quality')}</span>
                <div className="quality-tabs">
                  {(['draft', 'standard', 'high', 'custom'] as const).map((value) => (
                    <button key={value} className={quality === value ? 'active' : ''} onClick={() => setQuality(value)} disabled={exporting}>
                      {t(`export.quality.${value}` as TranslationKey)}
                    </button>
                  ))}
                </div>
                {quality === 'custom' && (
                  <div className="advanced-quality">
                    <label>
                      <span>{t('export.rateMode')}</span>
                      <select value={rateMode} onChange={(event) => setRateMode(event.target.value as ExportOptions['rateMode'])}>
                    ...
                    {rateMode === 'crf' ? (
                      <label>
                        <span>CRF (16–32)</span>
                        <input type="number" min={16} max={32} value={crf} onChange={(event) => setCrf(Number(event.target.value))} />
                      </label>
                    ) : (
                      <label>
            ...
            <div className="export-section">
              <span className="export-label">{t('export.scope')}</span>
              <div className="scope-toggle">
                <button className={scope === 'all' ? 'active' : ''} onClick={() => setScope('all')} disabled={exporting}>
                  {t('export.allTimeline')}
                </button>
                <button className={scope === 'range' ? 'active' : ''} onClick={() => setScope('range')} disabled={exporting || rangeStart === null || rangeEnd === null}>
                  In–Out {rangeStart !== null && rangeEnd !== null ? `(${formatTime(rangeEnd - rangeStart)})` : t('common.notSet')}
                </button>
              </div>
            </div>
            <label className="export-file-name">
              <span>{t('export.fileName')}</span>
              <input value={fileName} onChange={(event) => setFileName(event.target.value)} disabled={exporting} />
            </label>
          </div>
          <aside className="export-summary">
            <div className="summary-icon">↗</div>
            <strong>{isVideo ? t('export.videoExport') : t('export.audioExport')}</strong>
            <p>{isVideo ? `${aspect} · ${outputHint} · ${fps} FPS` : format === 'mp3' ? `MP3 · ${audioBitrateKbps} kbps` : 'WAV · PCM'}</p>
            {isVideo && (
            ...
            {preflight?.warnings.map((warning) => (
              <div className="export-warning" key={warning.code}>
                ⚠ {warning.message}
              </div>
            ))}
            {status.status && (
              <div className="export-progress">
                <div className="progress-head">
                  ...
                  <b>{Math.round(status.progress * 100)}%</b>
                </div>
                <div className="progress-track">
                  <i style={{ width: `${Math.max(2, status.progress * 100)}%` }} />
                </div>
              </div>
            )}
            {status.error && status.error !== error && <div className="export-error">{status.error}</div>}
            {done && status.downloadUrl && (
              <div className="export-complete">
                <span>✓ {t('common.ready')}</span>
                <strong>{status.fileName}</strong>
                <a className="secondary-button" href={status.downloadUrl} download={status.fileName}>
                  {t('common.download')}
                </a>
              </div>
            )}
        ...
        {error && (
          <div className="export-error export-error-bottom">
            {error}
            {!hasTimelineClips && (
              <button
                className="secondary-button export-recovery-button"
                onClick={() => {
                  if (onAddFirstAsset()) setError('');
                }}
              >
                {t('export.addLibraryMedia')}
              </button>
            )}
          </div>
        )}
        <div className="modal-actions">
          <span>
            {preflight?.estimatedBytes
              ? t('export.estimatedSize', {
                  size: (preflight.estimatedBytes / 1024 / 1024).toFixed(1),
                })
              : t('export.localFfmpeg')}
          </span>
...
function EditorTopbar({ project, onBack, backPending, onExport, exporting, onSettings, onCommands }: { project: Project; onBack: () => void; backPending: boolean; onExport: () => void; exporting: boolean; onSettings: () => void; onCommands: () => void }) {
  ...
  const saveLabel = saveState === 'saving' ? t('common.saving') : saveState === 'error' ? t('common.saveError') : saveState === 'offline' ? t('editor.status.offlinePending') : saveTime ? `${t('common.saved')} · ${saveTime}` : t('common.saved');
  const saveTitle = saveState === 'saved' && saveTime ? t('editor.status.lastSavedAt', { time: saveTime }) : saveLabel;
  return (
    <header className="editor-topbar">
      <div className="topbar-left">
        <button className="back-button" disabled={backPending} aria-busy={backPending} onClick={onBack}>
          &#8249;
        </button>
      ...
      <div className="topbar-center">
        <button className="history-button" onClick={undo} title={t('common.undo')}>
          &#8630;
        </button>
        <button className="history-button" onClick={redo} title={t('common.redo')}>
          &#8631;
        </button>
        <button className="topbar-command-button" onClick={onCommands} title={t('command.open')}>
          <span>&#8981;</span>
          <small>{t('command.title')}</small>
          <kbd>&#8984; K</kbd>
        </button>
        <span className={'save-indicator ' + saveState} title={saveTitle} aria-live="polite">
          <i className={'status-dot ' + saveState} /> {saveLabel}
        </span>
      </div>
      <div className="topbar-right">
        <ThemeSwitcher compact />
        <button className="export-button" disabled={exporting} onClick={onExport}>
          {exporting ? t('common.exporting') : t('common.export')} <Glyph>&#8599;</Glyph>
        </button>
        <button className="icon-button editor-settings" onClick={onSettings} title={t('common.settings')}>
          <Glyph>&#9881;</Glyph>
        </button>
        <button className="avatar-button" onClick={onSettings} title={t('common.settings')}>
          HK
        </button>
      </div>
    </header>
...
export function SettingsModal({ settings, onClose }: { settings: Settings | null; onClose: () => void }) {
      ...
      <section className="settings-section">
        <div className="settings-section-heading">
          ...
          <div>
            ...
            <small>{t('settings.interfaceHint')}</small>
          </div>
        </div>
        <label className="setting-row">
          ...
          <select aria-label={t('settings.language')} value={form.language} onChange={(event) => setForm({ ...form, language: event.target.value as 'en' | 'tr' })}>
        ...
        <label className="setting-row">
          <span>
            <strong>{t('settings.previewQuality')}</strong>
            <small>{t('settings.previewQualityHint')}</small>
          </span>
          <select
            value={form.proxyQuality}
            onChange={(event) =>
              setForm({
                ...form,
                proxyQuality: event.target.value as 'draft' | 'balanced' | 'high',
              })
            }
          >
            <option value="draft">{t('settings.previewQuality.draft')}</option>
            <option value="balanced">{t('settings.previewQuality.balanced')}</option>
            <option value="high">{t('settings.previewQuality.high')}</option>
          </select>
        </label>
      </section>
      <section className="settings-section">
        <div className="settings-section-heading">
          ...
          <div>
            <strong>{t('settings.exportDefaults')}</strong>
            <small>{t('settings.exportDefaultsHint')}</small>
          </div>
        </div>
        <div className="settings-control-grid">
          <label>
            <span>{t('export.format')}</span>
            <select
              value={form.defaultExport.format}
              onChange={(event) =>
                setForm({
                  ...form,
                  defaultExport: {
                    ...form.defaultExport,
                    format: event.target.value as typeof form.defaultExport.format,
                  },
                })
              }
            >
          ...
          <label>
            ...
            <select
              value={form.defaultExport.resolution}
              onChange={(event) =>
                setForm({
                  ...form,
                  defaultExport: {
                    ...form.defaultExport,
                    resolution: event.target.value as typeof form.defaultExport.resolution,
                  },
                })
              }
            >
          ...
          <label>
            ...
            <select
              value={form.defaultExport.fps}
              onChange={(event) =>
                setForm({
                  ...form,
                  defaultExport: {
                    ...form.defaultExport,
                    fps: Number(event.target.value) as typeof form.defaultExport.fps,
                  },
                })
              }
            >
          ...
          <label>
            <span>{t('export.quality')}</span>
            <select
              value={form.defaultExport.quality}
              onChange={(event) =>
                setForm({
                  ...form,
                  defaultExport: {
                    ...form.defaultExport,
                    quality: event.target.value as typeof form.defaultExport.quality,
                  },
                })
              }
            >
              ...
              <option value="custom">{t('export.quality.custom')}</option>
            </select>
          </label>
        </div>
        <div className="setting-row setting-readonly">
          <span>
            <strong>{t('settings.encoder')}</strong>
            <small>{t('settings.encoderHint')}</small>
          </span>
          <b>
            <i /> H.264 · CPU
          </b>
      ...
      <section className="settings-section settings-layout-section">
        <div className="settings-section-heading">
          ...
          <div>
            ...
            <small>{t('settings.layoutHint')}</small>
          </div>
        </div>
        <button
          type="button"
        ...
        >
          <span>↺</span>
          <div>
            <strong>{t('settings.resetLayout')}</strong>
            <small>{t('settings.resetLayoutHint')}</small>
          </div>
        </button>
      </section>
    </div>
  ...
  return (
    ...
    >
      <section className="settings-modal settings-modal-v2" role="dialog" aria-modal="true" aria-labelledby="settings-title">
        <div className="modal-head settings-modal-head">
          <div>
            <p className="eyebrow">{t('settings.workspace')}</p>
            <h2 id="settings-title">{t('settings.title')}</h2>
            <small>{t('settings.copy')}</small>
          </div>
            ...
            {activeTab === 'shortcuts' && (
              <div className="shortcut-settings">
                <div className="settings-intro">
                  <strong>{t('settings.editShortcuts')}</strong>
                  <small>{t('settings.shortcutHint')}</small>
                </div>
                {(Object.keys(SHORTCUT_LABELS) as ShortcutAction[]).map((action) => {
                  const label = t(SHORTCUT_LABELS[action].labelKey);
                  return (
                    <div className="shortcut-setting-row" key={action}>
                      <span>
                        <strong>{label}</strong>
                        <small>{t(SHORTCUT_LABELS[action].descriptionKey)}</small>
                      </span>
                      <kbd aria-label={t('settings.shortcutAria', { label })}>{DEFAULT_SHORTCUTS[action]}</kbd>
                    </div>
                  );
                })}
              </div>
...
function WorkspaceResizers({ layout, onPreview, onCommit }: { layout: WorkspaceLayout; onPreview: (next: WorkspaceLayout) => void; onCommit: (next: WorkspaceLayout) => void }) {
  ...
  return (
    <>
      {resizer('rail', 'workspace-resizer-rail', t('workspace.resizeRail'), 'vertical')}
      {resizer('library', 'workspace-resizer-library', t('workspace.resizeLibrary'), 'vertical')}
      {resizer('inspector', 'workspace-resizer-inspector', t('workspace.resizeInspector'), 'vertical')}
      {resizer('timeline', 'workspace-resizer-timeline', t('workspace.resizeTimeline'), 'horizontal')}
    </>
  );
}
...
function ToolRail({ onOpenSettings }: { onOpenSettings: () => void }) {
  ...
  const tools: Array<[Panel, string, TranslationKey]> = [
    ['media', '▧', 'editor.panel.media'],
    ['text', 'T', 'editor.panel.text'],
    ['elements', '◇', 'editor.panel.elements'],
    ['project', '◉', 'editor.panel.project'],
  ];
  return (
    <aside className="tool-rail" aria-label={t('editor.tools')}>
