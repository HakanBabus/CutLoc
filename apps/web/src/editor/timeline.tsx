...
export function TimelinePro({ project }: { project: Project }) {
  ...
  const seek = (event: React.MouseEvent<HTMLElement>) => { setCurrentTime(frameTime(timeFromClientX(event.clientX))); };
  ...
  const addLayerRelative = (trackId: string, offset: -1 | 1) => { mutateProject((draft) => { const index = draft.tracks.findIndex((item) => item.id === trackId); if (index < 0) return; createLayerTrack(draft); const [layer] = draft.tracks.splice(draft.tracks.length - 1, 1); draft.tracks.splice(Math.max(0, Math.min(draft.tracks.length, index + (offset < 0 ? 0 : 1))), 0, layer); draft.tracks.forEach((item, order) => { item.order = order; }); }); closeMenu(); };
  ...
  return <section className="timeline timeline-pro" onContextMenu={(event) => { event.preventDefault(); const target = event.target as HTMLElement; if (target.closest('.timeline-clip,.track-label,.timeline-marker,.context-menu')) return; setMenu({ x: event.clientX, y: event.clientY, kind: 'empty', time: snapTime(timeFromClientX(event.clientX)) }); }}>
    <div className="timeline-toolbar">
      <div className="timeline-toolbar-left">
        <button className="timeline-tool active" title={t('timeline.selection')} aria-label={t('timeline.selection')} onClick={() => setSelected(null, null)}><UiIcon name="cursor" /></button>
        <button className="timeline-tool" title={canSplit ? t('timeline.split') : t('timeline.splitHint')} aria-label={t('timeline.split')} disabled={!canSplit} onClick={() => selectedClipId && splitClip(selectedClipId)}><UiIcon name="scissors" /></button>
        <button className="timeline-tool" title={t('common.undo')} aria-label={t('common.undo')} disabled={!canUndo} onClick={undo}><UiIcon name="undo" /></button>
        <button className="timeline-tool" title={t('common.redo')} aria-label={t('common.redo')} disabled={!canRedo} onClick={redo}><UiIcon name="redo" /></button>
        <button className={`timeline-tool snap-toggle ${snapEnabled ? 'active' : ''}`} title={t(snapEnabled ? 'timeline.snapOn' : 'timeline.snapOff')} aria-label={t(snapEnabled ? 'timeline.snapOn' : 'timeline.snapOff')} aria-pressed={snapEnabled} onClick={() => setSnapEnabled((value) => !value)}><UiIcon name="snap" /></button>
        <button className="timeline-tool" title={t('timeline.addMarker')} aria-label={t('timeline.addMarker')} onClick={addMarker}><UiIcon name="marker" /></button>
        <button className="track-add-button" onClick={(event) => setMenu({ x: event.clientX, y: event.clientY, kind: 'add-track' })}><UiIcon name="plus" /> <span>{t('timeline.addTrack')}</span></button>
      </div>
      <div className="timeline-toolbar-right"><span className="zoom-label">{t('timeline.zoom')} · {Math.round(px)} px/s</span><input aria-label={t('timeline.zoom')} type="range" min="38" max="260" value={px} onChange={(event) => setZoom(Number(event.target.value))} /></div>
    </div>
    <div className="timeline-fit-row"><button className="timeline-fit-button" onClick={fitTimeline}><UiIcon name="fit" /> <span>{t('timeline.fit')}</span></button><span>{t('timeline.nudgeHint')}</span></div>
