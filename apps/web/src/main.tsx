...
function Dashboard({ projects, trash, loading, onCreate, onStartWithMedia, onOpen, onDelete, onRestoreTrash, onPurgeTrash, onSettings, onImportBundle }: { projects: DashboardProject[]; trash: TrashEntry[]; loading: boolean; onCreate: () => void; onStartWithMedia: (file: File) => void; onOpen: (id: string) => void; onDelete: (id: string) => void; onRestoreTrash: (trashId: string) => void; onPurgeTrash: (trashId: string) => void; onSettings: () => void; onImportBundle: (bundle: unknown) => void }) {
  ...
  return <main key={language} className="dashboard">
    <header className="dashboard-header">
      <div className="brand"><img className="brand-logo" src="/favicon.svg" alt="" /><div><strong>CutLoc</strong><small>Video editor</small></div></div>
    ...
    <section className="dashboard-hero">
      <div className="dashboard-hero-copy"><p className="eyebrow">CutLoc Studio</p><h1>{t('dashboard.hero.titleLead')} <em>{t('dashboard.hero.titleAccent')}</em> {t('dashboard.hero.titleTail')}</h1><p className="hero-copy">{t('dashboard.hero.copy')}</p><div className="hero-actions"><button className="primary-button large" onClick={onCreate}><Glyph>＋</Glyph> {t('dashboard.newProject')}</button><button className="secondary-button" onClick={() => mediaFileRef.current?.click()}>{t('dashboard.command.media')} <Glyph>↗</Glyph></button></div></div>
    ...
    <section className="dashboard-command-strip" aria-label={t('dashboard.quickStart')}>
      <span className="quick-action-label">{t('dashboard.quickStart')}</span>
      <button className="command-card" onClick={onCreate}><span className="command-icon">＋</span><span><strong>{t('dashboard.command.new')}</strong><small>{t('dashboard.command.newHint')}</small></span></button>
      <button className="command-card" onClick={() => mediaFileRef.current?.click()}><span className="command-icon">▣</span><span><strong>{t('dashboard.command.media')}</strong><small>{t('dashboard.command.mediaHint')}</small></span></button>
      {projects[0] && <button className="command-card" onClick={() => onOpen(projects[0].id)}><span className="command-icon">▶</span><span><strong>{t('dashboard.command.continue')}</strong><small>{projects[0].name} · {formatTime(projects[0].duration)}</small></span></button>}
    </section>
    ...
    <footer className="dashboard-footer"><span>CutLoc Studio</span><span><b>v0.1.0 beta</b></span></footer>
...
function ProjectCard({ project, onOpen, onDelete }: { project: DashboardProject; onOpen: () => void; onDelete: () => void }) {
  ...
  return <article className="project-card" onDoubleClick={onOpen}>
    <button className={`project-preview ${accent}`} onClick={onOpen} aria-label={project.name}>{coverUrl && <img className="project-preview-media" src={coverUrl} alt="" loading="lazy" onError={(event) => { event.currentTarget.hidden = true; }} />}<div className="preview-grid" /><span className="project-play">▶</span><span className="project-open-label">{t('dashboard.openProject')}</span><span className="aspect-tag">{project.canvas.aspect}</span></button>
