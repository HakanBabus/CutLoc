...
const useEditor = create<EditorState>((set) => ({
  ...
  setProject: (project, resetHistory = true) => set((state) => resetHistory
    ? { project, history: { past: [], future: [] }, historyGroup: null, selectedClipId: null, selectedClipIds: [], selectedTrackId: null, currentTime: 0, rangeStart: null, rangeEnd: null, assetDragId: null, saveState: 'saved', notice: '' }
    : { ...state, project }),
...
function App() {
  ...
  return <div className="app-shell" data-theme={theme}>
    <div className={viewClass} key={screen}>
      {screen === 'dashboard' ? <Dashboard projects={projects} trash={trash} loading={loading} onCreate={createProject} onOpen={openProject} onDelete={requestDeleteProject} onRestoreTrash={restoreTrash} onPurgeTrash={purgeTrash} onSettings={() => setShowSettings(true)} onImportBundle={importBundle} /> : project ? <Editor onBack={returnToDashboard} /> : null}
    </div>
...
function Dashboard({ projects, trash, loading, onCreate, onOpen, onDelete, onRestoreTrash, onPurgeTrash, onSettings, onImportBundle }: { projects: Project[]; trash: TrashEntry[]; loading: boolean; onCreate: () => void; onOpen: (id: string) => void; onDelete: (id: string) => void; onRestoreTrash: (trashId: string) => void; onPurgeTrash: (trashId: string) => void; onSettings: () => void; onImportBundle: (bundle: unknown) => void }) {
...
function Editor({ onBack }: { onBack: () => void }) {
  ...
  const startExport = async (options: ExportOptions): Promise<ExportPreflight> => {
    ...
    try {
      ...
      setExporting(false);
      setExportStatus({ progress: 0, status: 'failed', error: error instanceof Error ? error.message : t('export.failed') });
      throw error;
    }
  };
